...
public class ATReport implements HRReport {
	...
	public String AtToString() {
		...
		return (sd.format(d) + "\t" + getEmpId() + "\t\t\t" + getEmpName() + "\t\t\t" + getTimeIn() + "\t\t"
				+ getTimeOut() + "\n");
	}
	...
	public void reportByDepartment(String department) {
		...
		if (!myNewList.isEmpty()) {
			...
			System.out.println(
					"FORM CODE\t\t\tATTENDANCE DATE\t\tTIME IN\t\tTIME OUT\tSTATUS\n-----------------------------------------------------------------------------------------------");
			for (ATForm e : myNewList) {
				System.out.println(sb
						.append(e.getFormCode() + "\t" + sd.format(e.getATDate()) + "\t\t" + e.getTimeIn() + "\t\t"
								+ e.getTimeOut() + "\t\t" + e.getStatus() + "\n")
