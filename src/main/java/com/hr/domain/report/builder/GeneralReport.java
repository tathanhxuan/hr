...
public class GeneralReport {
	...
	private GeneralReport(Builder builder) {
		...
		if (this.aTReport != null && !this.aTReport.isEmpty()) {
			...
			System.out.println(
					"Date\t\tEmployee ID\t\tEmployee Name\t\tTime In\t\tTime Out\n-----------------------------------------------------------------------------------------");
		...
		if (this.oTReport != null && !this.oTReport.isEmpty()) {
			...
			System.out.println(
					"Date\t\tEmployee ID\tEmployee Name\t\t\tStart Time\tEnd Time\n------------------------------------------------------------------------------------------");
		...
		if (this.leaveReport != null && !this.leaveReport.isEmpty()) {
			...
			System.out.println(
					"Date\t\tEmployee ID\tEmployee Name\t\tLeave Start\tLeave End\tApprove Status\tDescription\n----------------"
							+ "---------------------------------------------------------------------------------------------------");
			...
			for (ATForm atForm : allATForm) {
				...
				for (Employee emp : allEmployee) {
					if (empId.equals(emp.getEmpID())) {
						...
						ATReport a = new ATReport(atForm.getATDate(), emp.getEmpID(), emp.getEmpName(),
								atForm.getTimeIn(), atForm.getTimeOut());
			...
			for (OTForm otForm : allOTForm) {
				...
				for (Employee emp : allEmployee) {
					if (empId.equals(emp.getEmpID())) {
						...
						OTReport a = new OTReport(emp.getEmpID(), emp.getEmpName(), otForm.getTimeTo(),
								otForm.getTimeFrom(), otForm.getOTDate());
			...
			for (LeaveForm lvForm : allLeaveForm) {
				...
				for (Employee emp : allEmployee) {
					if (empId.equals(emp.getEmpID())) {
						...
						LeaveReport a = new LeaveReport(lvForm.getLeaveDateTo(), empid, empname, dateFrom, dateTo,
								lvForm.getStatus().toString(), lvForm.getDescription());
